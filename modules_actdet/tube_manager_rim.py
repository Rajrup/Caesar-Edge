...
class TubeManager:
    ...
    def getTubeInput(self, request, frame_id, deepsort_output):
      # print("[Yitao] @@@ debug @@@, deepsort_output = %s" % deepsort_output)
      output = {'img': request, 'meta': {'frame_id': frame_id}}
      output['meta']['obj'] = []
    ...
    def PreProcess(self, request, istub, tube_manager, my_lock, grpc_flag):
        self.istub = istub
        ...
        if (grpc_flag):
            ...
            deepsort_output = request['deepsort_output']
            self.frame_id = int(request['frame_info'].split('-')[-1])
    ...
    def PostProcess(self, grpc_flag):
        if (grpc_flag):
            ...
            next_request = dict()
            next_request['frames_output'] = frames_output
            next_request['temporal_rois_output'] = temporal_rois_output
            next_request['norm_rois_output'] = norm_rois_output
            next_request['actor_boxes_output'] = actor_boxes_output
