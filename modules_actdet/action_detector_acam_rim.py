...
class ACAM:
    ...
    def PreProcess(self, request, istub, grpc_flag):
        ...
        if (grpc_flag):
            ...
            if (str(request["frames_output"]) == "None"):
                return
            else:
                self.has_input = True
                self.frames = request["frames_output"]
                self.temporal_rois = request["temporal_rois_output"]
                self.norm_rois = request["norm_rois_output"]
                self.actor_boxes = request["actor_boxes_output"]
